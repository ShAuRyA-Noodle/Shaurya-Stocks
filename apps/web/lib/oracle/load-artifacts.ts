...
/**
 * Load pre-generated ML signals from oracle artifacts.
 * Generated by `quant ml predict` on real Alpaca 2018-2026 data.
 * Returns empty array (never throws) so the page degrades gracefully.
 */
export function loadStaticSignals(): readonly StaticSignal[] {
  for (const root of ARTIFACT_ROOTS) {
    const p = join(root, "signals.json")
    if (!existsSync(p)) continue
    try {
      const parsed = JSON.parse(readFileSync(p, "utf8")) as StaticSignal[]
